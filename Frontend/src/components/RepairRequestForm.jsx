...
function RepairRequestForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const responseJson = await fetchApi(`add_repair`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...formData,
          services: formData.services.join(","), // Convert array to comma-separated string for backend
        }),
      });

      console.log("add_repair API raw response:", responseJson);

      // --- FIX START ---
      // The backend returns { success: true, data: true } on successful repair submission.
      // We need to check responseJson.success and then responseJson.data directly.
      if (responseJson.success && responseJson.data === true) { // Check if data is explicitly true
        ...
        // Handle cases where backend success is false or data is not true
        // Use backend's message if available, otherwise a generic one
        throw new Error(responseJson.message || "Une erreur est survenue lors de l'envoi de votre demande.");
      }
      // --- FIX END ---
    } catch (err) {
