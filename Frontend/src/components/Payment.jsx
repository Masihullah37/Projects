...
function CheckoutForm({ amount, cartItems, user, clearCart, navigate }) {
  ...
  const getOrCreateAchat = async (userId) => {
    try {
      ...
      // fetchApi now returns the full JSON response, no need for .ok check or .json() call here.
      const responseJson = await fetchApi(`get_or_create_achat`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          // csrf_token: user.csrf_token // Include CSRF token
        }),
      })

      // console.log("Résultat de création d'achat (raw):", responseJson)

      // Check the nested 'success' flag from the backend's data
      if (responseJson.success && responseJson.data && responseJson.data.achat_id) {
        return responseJson.data.achat_id
      } else {
        // Handle logical error from backend
        throw new Error(responseJson.data.message || "Erreur lors de la création de l'achat.");
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const paymentData = {
        ...
        cart_items: cartItems, // Pass the cart items from context
        csrf_token: user.csrf_token // Include CSRF token
      };
      ...
      const result = await fetchApi(`add_payment`, {
        method: "POST",
      ...
      // console.log("Résultat du paiement (raw):", result); // Keep this log

      // Check if the overall response is successful and contains the 'data' object
      if (result.error || !result.data || !result.data.client_secret) {
        console.error("Backend response missing client_secret or not successful:", result);
        throw new Error(result.error || "Client secret missing from backend response.");
      }

      const clientSecret = result.data.client_secret; // Extract client_secret correctly
      const paymentId = result.data.payment_id;       // Extract payment_id correctly

      // console.log("Client Secret for Stripe:", clientSecret);
      // console.log("Payment ID from backend:", paymentId);

      // Confirmation du paiement avec Stripe
      const { error: confirmError, paymentIntent } = await stripe.confirmCardPayment(clientSecret, { // Use clientSecret here
        payment_method: paymentMethod.id,
      });

      if (confirmError) {
        console.error("Erreur de confirmation Stripe:", confirmError.message);
        throw new Error(confirmError.message);
      }

      // console.log("Paiement confirmé (Stripe PaymentIntent):", paymentIntent);

      // Mise à jour du statut du paiement
      // fetchApi now returns the full JSON response, no need for .json() call here.
      const updateResult = await fetchApi(`update_payment_status`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: 'include',
          body: JSON.stringify({
            payment_id: paymentId, // Use the extracted paymentId
      ...
      // console.log("Résultat de mise à jour du statut:", updateResult);

      if (updateResult.error) {
        console.error("Erreur de mise à jour du statut:", updateResult.error);
      }
